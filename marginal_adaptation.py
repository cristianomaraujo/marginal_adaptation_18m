# -*- coding: utf-8 -*-
"""Fratura dentística - Thalita.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1wxvpiBRUJFEoEN_-DWYn1MXrq4RPkodo
...
colunas = [ 'Age', 'Type of adhesive', 'Tooth distribution',
       ...
       'Marginal adaptation 18m (FDI)(1=S/2=N)']


df = df.loc[:, colunas]

coluna_grupo = df.pop('Marginal adaptation 18m (FDI)(1=S/2=N)')
df.insert(0, 'Marginal adaptation 18m (FDI)(1=S/2=N)', coluna_grupo)
...
X = df.drop('Marginal adaptation 18m (FDI)(1=S/2=N)', axis=1)
y = df['Marginal adaptation 18m (FDI)(1=S/2=N)']

df['Marginal adaptation 18m (FDI)(1=S/2=N)'].value_counts()
...
# RFE
rfe = RFECV(estimator=best_grd_boost, cv=5, scoring='accuracy')
rfe.fit(X_train, y_train)
X_train_rfe = rfe.transform(X_train)
X_test_rfe = rfe.transform(X_test)

# Train and predict with best model on reduced feature set
best_grd_boost.fit(X_train_rfe, y_train)
y_pred_test_gb = best_grd_boost.predict(X_test_rfe)
...
y_pred_cv_gb = cross_val_predict(best_grd_boost, X_train_rfe, y_train, cv=kf_gb)
...
# RFE
rfe = RFECV(estimator=best_log_reg, cv=5, scoring='accuracy')
rfe.fit(X_train, y_train)
X_train_rfe = rfe.transform(X_train)
X_test_rfe = rfe.transform(X_test)
...
y_pred_test_lr = best_log_reg.predict(X_test_rfe)
...
y_pred_cv_lr = cross_val_predict(best_log_reg, X_train_rfe, y_train, cv=kf_lr)
...
# RFE
rfe = RFECV(estimator=best_tree_clf, cv=5, scoring='accuracy')
rfe.fit(X_train, y_train)
X_train_rfe = rfe.transform(X_train)
X_test_rfe = rfe.transform(X_test)
...
# Cross-validation no conjunto de características reduzido
...
imp_features_dt2 = []
df_imp_features_dt2 = []
X_2 = df.drop('Marginal adaptation 18m (FDI)(1=S/2=N)', axis=1)
features_2 = []
...
plt.figure(figsize=(10, 6))
colors = plt.cm.Oranges(np.linspace(0.2, 1, len(df_imp_features_dt2_sorted)))
...
# RFE
rfe = RFECV(estimator=best_forest_clf, cv=5, scoring='accuracy')
rfe.fit(X_train, y_train)
X_train_rfe = rfe.transform(X_train)
X_test_rfe = rfe.transform(X_test)

best_forest_clf.fit(X_train_rfe, y_train)
y_pred_test_rf = best_forest_clf.predict(X_test_rfe)
...
y_pred_cv_rf = cross_val_predict(best_forest_clf, X_train_rfe, y_train, cv=kf_rf)
...
X_3 = df.drop('Marginal adaptation 18m (FDI)(1=S/2=N)', axis=1)
features_3 = []
