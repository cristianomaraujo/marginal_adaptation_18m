# -*- coding: utf-8 -*-
"""Fratura dentística Final IC95% - Thalita.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1gPB1P2dElQkHcnO7bLsK5i-NEXtjrzIg
...
colunas = [ 'Age', 'Type of adhesive', 'Tooth distribution',
        'Shape (degree of angle)',
       'Score on dentin sclerosis scale','Inciso-gingival height (mm)','Arch distribution',
       'Marginal adaptation 18m (FDI)(1=S/0=N)']


df = df.loc[:, colunas]

coluna_grupo = df.pop('Marginal adaptation 18m (FDI)(1=S/0=N)')
df.insert(0, 'Marginal adaptation 18m (FDI)(1=S/0=N)', coluna_grupo)
...
X = df.drop('Marginal adaptation 18m (FDI)(1=S/0=N)', axis=1)
y = df['Marginal adaptation 18m (FDI)(1=S/0=N)']

df['Marginal adaptation 18m (FDI)(1=S/0=N)'].value_counts()
...
grid_search = GridSearchCV(estimator=gb_model, param_grid=param_grid, cv=5, scoring='precision')
...
# RFE
rfe = RFECV(estimator=best_gb_model, cv=5, scoring='precision')
rfe.fit(X_train, y_train)
X_train_rfe = rfe.transform(X_train)
X_test_rfe = rfe.transform(X_test)
best_gb_model.fit(X_train_rfe, y_train)
y_pred_test_gb = best_gb_model.predict(X_test_rfe)
y_prob_test_gb = best_gb_model.predict_proba(X_test_rfe)[:, 1]
...
metrics_test = bootstrap_metrics(best_gb_model, X_test_rfe, y_test)
...
y_prob_cv_gb = cross_val_predict(best_gb_model, X_train_rfe, y_train, cv=kf_gb, method='predict_proba')[:, 1]
y_pred_cv_gb = cross_val_predict(best_gb_model, X_train_rfe, y_train, cv=kf_gb)
...
for train_index, test_index in kf_gb.split(X_train_rfe):
    X_train_fold, X_test_fold = X_train_rfe[train_index], X_train_rfe[test_index]
...
auc_scores_test_gb = bootstrap_auc(best_gb_model, X_test_rfe, y_test)
auc_ci_test_gb = np.percentile(auc_scores_test_gb, [2.5, 97.5])

print(f"Test Data AUC: {roc_auc_gb} (95% CI: {auc_ci_test_gb})")

def auc_cross_val(model, X, y, cv):
    auc_scores = []
    for train_index, test_index in cv.split(X):
        model.fit(X[train_index], y[train_index])
        y_prob = model.predict_proba(X[test_index])[:, 1]
        fpr, tpr, _ = roc_curve(y[test_index], y_prob)
        auc_scores.append(auc(fpr, tpr))
    return auc_scores

auc_scores_cv_gb = auc_cross_val(best_gb_model, X_train_rfe, y_train, kf_gb)
...
colors = plt.cm.Oranges(np.linspace(0.2, 1, len(feature_importance_sorted)))
...
grid_search = GridSearchCV(estimator=logreg_model, param_grid=param_grid, cv=5, scoring='precision')
grid_search.fit(X_train, y_train)
print("Best Parameters:", grid_search.best_params_)
print("Best Score:", grid_search.best_score_)
...
# RFE
rfe = RFECV(estimator=best_logreg_model, cv=5, scoring='precision')
rfe.fit(X_train, y_train)
X_train_rfe = rfe.transform(X_train)
X_test_rfe = rfe.transform(X_test)

best_logreg_model.fit(X_train_rfe, y_train)
y_pred_test_logreg = best_logreg_model.predict(X_test_rfe)
y_prob_test_logreg = best_logreg_model.predict_proba(X_test_rfe)[:, 1]
...
metrics_test = bootstrap_metrics(best_logreg_model, X_test_rfe, y_test)
...
y_prob_cv_logreg = cross_val_predict(best_logreg_model, X_train_rfe, y_train, cv=kf_logreg, method='predict_proba')[:, 1]
y_pred_cv_logreg = cross_val_predict(best_logreg_model, X_train_rfe, y_train, cv=kf_logreg)
...
for train_index, test_index in kf_logreg.split(X_train_rfe):
    X_train_fold, X_test_fold = X_train_rfe[train_index], X_train_rfe[test_index]
    y_train_fold, y_test_fold = y_train[train_index], y_train[test_index]
...
auc_scores_test_logreg = bootstrap_auc(best_logreg_model, X_test_rfe, y_test)
auc_ci_test_logreg = np.percentile(auc_scores_test_logreg, [2.5, 97.5])

print(f"Test Data AUC: {roc_auc_logreg} (95% CI: {auc_ci_test_logreg})")
...
auc_scores_cv_logreg = auc_cross_val(best_logreg_model, X_train_rfe, y_train, kf_logreg)

auc_ci_cv_logreg = ci95(auc_scores_cv_logreg)

print(f"Cross-Validation AUC: {np.mean(auc_scores_cv_logreg)} (95% CI: {auc_ci_cv_logreg})")
...
feature_importance_lr = pd.DataFrame({'Feature': selected_features, 'Importance': np.abs(coefficients)})
feature_importance_lr = feature_importance_lr.sort_values('Importance', ascending=True)
colors = plt.cm.Oranges(np.linspace(0.2, 1, len(feature_importance_lr)))
plt.figure(figsize=(10, 6))
...
grid_search = GridSearchCV(estimator=knn_model, param_grid=param_grid, cv=5, scoring='precision')
grid_search.fit(X_train, y_train)
print("Best Parameters:", grid_search.best_params_)
print("Best Score:", grid_search.best_score_)
...
def auc_cross_val(model, X, y, cv, pos_label=2):
    ...
    for train_index, test_index in cv.split(X):
        ...
        fpr, tpr, _ = roc_curve(y.iloc[test_index], y_prob, pos_label=pos_label)
        auc_scores.append(auc(fpr, tpr))
    return auc_scores
...
grid_search = GridSearchCV(estimator=mlp_model, param_grid=param_grid, cv=5, scoring='precision')
grid_search.fit(X_train, y_train)
print("Best Parameters:", grid_search.best_params_)
print("Best Score:", grid_search.best_score_)
...
grid_search = GridSearchCV(estimator=tree_clf, param_grid=param_grid, cv=5, scoring='precision')
grid_search.fit(X_train, y_train)
print("Best Parameters:", grid_search.best_params_)
print("Best Score:", grid_search.best_score_)
...
# RFE
rfe = RFECV(estimator=best_tree_clf, cv=5, scoring='precision')
rfe.fit(X_train, y_train)
X_train_rfe = rfe.transform(X_train)
X_test_rfe = rfe.transform(X_test)

best_tree_clf.fit(X_train_rfe, y_train)
y_pred_test_dt = best_tree_clf.predict(X_test_rfe)
y_prob_test_dt = best_tree_clf.predict_proba(X_test_rfe)[:, 1]
...
metrics_test = bootstrap_metrics(best_tree_clf, X_test_rfe, y_test)
...
y_prob_cv_dt = cross_val_predict(best_tree_clf, X_train_rfe, y_train, cv=kf_dt, method='predict_proba')[:, 1]
y_pred_cv_dt = cross_val_predict(best_tree_clf, X_train_rfe, y_train, cv=kf_dt)
...
for train_index, test_index in kf_dt.split(X_train_rfe):
    X_train_fold, X_test_fold = X_train_rfe[train_index], X_train_rfe[test_index]
    y_train_fold, y_test_fold = y_train[train_index], y_train[test_index]
...
auc_scores_test_dt = bootstrap_auc(best_tree_clf, X_test_rfe, y_test)
...
auc_scores_cv_dt = auc_cross_val(best_tree_clf, X_train_rfe, y_train, kf_dt)
...
#FEATURE IMPORTANCE
if len(selected_features) != len(imp_features_dt2):
    print("Os comprimentos das listas são diferentes. Verifique os dados.")
    print("Características selecionadas:", selected_features)
    print("Importâncias das características:", imp_features_dt2)
...
# RFE
rfe = RFECV(estimator=best_rf_model, cv=5, scoring='precision')
rfe.fit(X_train, y_train)
X_train_rfe = rfe.transform(X_train)
X_test_rfe = rfe.transform(X_test)

# Train and predict with best model on reduced feature set
best_rf_model.fit(X_train_rfe, y_train)
y_pred_test_rf = best_rf_model.predict(X_test_rfe)
y_prob_test_rf = best_rf_model.predict_proba(X_test_rfe)[:, 1]
...
metrics_test = bootstrap_metrics(best_rf_model, X_test_rfe, y_test)
...
y_prob_cv_rf = cross_val_predict(best_rf_model, X_train_rfe, y_train, cv=kf_rf, method='predict_proba')[:, 1]
y_pred_cv_rf = cross_val_predict(best_rf_model, X_train_rfe, y_train, cv=kf_rf)
...
for train_index, test_index in kf_rf.split(X_train_rfe):
    X_train_fold, X_test_fold = X_train_rfe[train_index], X_train_rfe[test_index]
    y_train_fold, y_test_fold = y_train[train_index], y_train[test_index]
...
auc_scores_test_rf = bootstrap_auc(best_rf_model, X_test_rfe, y_test)
...
auc_scores_cv_rf = auc_cross_val(best_rf_model, X_train_rfe, y_train, kf_rf, pos_label=1)
...
X_3 = df.drop('Marginal adaptation 18m (FDI)(1=S/0=N)', axis=1)
features_3 = []
...
plt.figure(figsize=(10, 6))
colors = plt.cm.Oranges(np.linspace(0.2, 1, len(df_imp_features_dt3_sorted)))
...
axs[0].plot(fpr_logreg, tpr_logreg, color='red', lw=2, label='Logistic Regression (AUC = %0.2f, CI 95%%: [%0.2f, %0.2f])' % (roc_auc_logreg, auc_ci_test_logreg[0], auc_ci_test_logreg[1]))
...
axs[1].plot(fpr_cv_logreg, tpr_cv_logreg, color='red', lw=2, label='Logistic Regression (AUC = %0.2f, CI 95%%: [%0.2f, %0.2f])' % (roc_auc_cv_logreg, auc_ci_cv_logreg[0], auc_ci_cv_logreg[1]))
...
model_names = ["gb", "knn", "svc", "mlp", "lr", "dt", "rf"]
auc_scores = {
    "gb": roc_auc_gb,
    "knn": roc_auc_knn,
    "svc": roc_auc_svc,
...
plt.tight_layout()
plt.savefig('Mean_difference.jpg', dpi=300, bbox_inches='tight')
plt.show()
...
colors = plt.cm.magma(np.linspace(0.2, 0.8, 4))  # Ajustando as cores para 4 modelos
...
# Garantir que todas as features sejam consideradas para o Decision Tree
imp_features_dt_full = np.zeros(len(features))
imp_features_dt_full[:len(imp_features_dt)] = imp_features_dt

# Ordenar as importâncias das features para todos os modelos
...
# Ordenar as importâncias das features para o Decision Tree em ordem decrescente
sorted_indices_dt_full = np.argsort(imp_features_dt_full)[::1]
...
# PLOT 3: Decision Tree (em ordem decrescente de importância)
axs[1, 0].barh(features[sorted_indices_dt_full], imp_features_dt_full[sorted_indices_dt_full], color=colors[2])
...
axs[1, 1].barh(features[sorted_indices_rf], imp_features_rf[sorted_indices_rf], color=colors[3])
